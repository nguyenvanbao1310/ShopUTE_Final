  ...
  useEffect(() => {
    async function fetchStats() {
      try {
        ...
        const ordersRes = await fetch(`${BASE_URL}/api/analytics/orders`);
        const ordersData = await ordersRes.json();

        // 🟢 2. Gọi API khách hàng mới
        const customersRes = await fetch(`${BASE_URL}/api/analytics/customers`);
        const customersData = await customersRes.json();

        // 🟠 Sản phẩm có sẵn
        const productsRes = await fetch(`${BASE_URL}/api/analytics/products`);
        ...
        setStats({
          totalOrders: {
            ...
            trendValue: `${Math.abs(Number(ordersData.trendValue || 0)).toFixed(
              2
            )}%`,
          },
          newCustomers: {
          ...
          availableProducts: {
            ...
            trend:
              (productsData.trend as "up" | "down" | "neutral") || "neutral",
            trendValue: `${Math.abs(
              Number(productsData.trendValue || 0)
            ).toFixed(2)}%`,
          },
        });
      } catch (error) {
