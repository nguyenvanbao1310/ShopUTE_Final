...
export class AnalyticsService {
  ...
  async getOrderCashFlow() {
    const [shipping, completed] = await Promise.all([
      this.orderRepo.query(
        `SELECT SUM(finalAmount) AS shipping FROM orders WHERE status = 'SHIPPED'`,
      ),
      this.orderRepo.query(
        `SELECT SUM(finalAmount) AS completed FROM orders WHERE status = 'COMPLETED'`,
      ),
    ]);
  ...
  async getTopProducts() {
    ...
    SELECT p.id, p.price, p.name, p.thumbnailUrl AS imageUrl, SUM(od.quantity) AS totalSold
